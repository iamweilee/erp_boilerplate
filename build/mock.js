...
function mockMiddleware(options) {
    ...
    function localMapping(reqPath, method) {
        var pathname = reqPath.replace(/^\/manage/, '');

        let paths = pathname.split('/');

        let surfix = paths[paths.length - 1];
        var findByIdReg = /^\d+$/;
        var matches;
        if (method === 'GET') {
            // if (findByIdReg.test(surfix)) {
            //     return pathname;
            // }
        ...
        if (method === 'PUT') {
            return pathname.replace(/\/\d+$/, '/update');
        }
        if (method === 'POST') {
    ...
    function resolveMockFilePath(reqPath) {
        return path.join(mockFilePath, 'mockup', localMapping(reqPath, method));
    }
...
function mockMiddlewareWrapper (options) {
...
router.get('*', mockMiddlewareWrapper(middlewareOptions)('GET'));
router.post('*', mockMiddlewareWrapper(middlewareOptions)('POST'));
router.put('*',  mockMiddlewareWrapper(middlewareOptions)('PUT'));
router.delete('*',  mockMiddlewareWrapper(middlewareOptions)('DELETE'));
